...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        const uploadDir = path.join(__dirname, "../../../uploads/images/our_story");
...
const toggleOurStoryStatus = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        ourStory.status = ourStory.status === "active" ? "inactive" : "active";
        await ourStory.save();

        res.status(200).json({
            success: true,
            message: "Our Story status updated successfully",
            data: ourStory,
        });
    } catch (error) {
        console.error("Error toggling Our Story status:", error);
        res.status(500).json({
            success: false,
            message: "Failed to toggle Our Story status",
            error: error.message,
        });
    }
};
