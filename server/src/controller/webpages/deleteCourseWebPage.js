    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            if (yogaTeamSlideImage) {
                const oldYogaImagePath = path.join(__dirname, "../uploads", courseWebpage.yogaTeamSlideImage);
            ...
            const updatedCourseWebpage = await CourseWebpages.findByIdAndUpdate(id, updateFields, { new: true });
...
const deleteCourseWebpage = async (req, res) => {
    try {
        ...
        const yogaImagePath = courseWebpage.yogaTeamSlideImage
            ? path.join(__dirname, "../uploads", courseWebpage.yogaTeamSlideImage)
            : null;
